...
_metal_kernel_jit_p = Primitive("mps.metal_kernel_jit")
...
_metal_kernel_lib_p = Primitive("mps.metal_kernel_lib")
...
def metal_kernel_lib(
    ...
    *,
    ...
    - `hash_name`: extra pipeline cache key. The kernel name and function-constant
      values are always part of the key, so distinct kernels and distinct
      specializations never share a cached pipeline.
