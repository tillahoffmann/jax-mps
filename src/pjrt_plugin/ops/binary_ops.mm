...
static ProcessResult HandleDotGeneral(HandlerContext& ctx) {
    ...
    // Handle rank-1 inputs: expand to rank-2 for matmul, then squeeze result.
    // This supports matrix-vector (2D x 1D), vector-matrix (1D x 2D),
    // vector-vector dot (1D x 1D with contraction), and outer product (1D x 1D
    // without contraction, generated by gradient of matvec).
    ...
    if (lhsBatchDims.empty() && rhsBatchDims.empty() && (lhsRank == 1 || rhsRank == 1) &&
        ...
        if (lhsRank == 1) {
            if (isOuterProduct) {
                ...
                // Contraction: (K,) -> (1, K), contracting dim shifts from 0 to 1
                lhs = [ctx.graph expandDimsOfTensor:lhs axis:0 name:nil];
            }
            lhsExpanded = true;
            lhsRank = 2;
        }
        if (rhsRank == 1) {
            if (isOuterProduct) {
                ...
                // Contraction: (K,) -> (K, 1), contracting dim stays at 0
                rhs = [ctx.graph expandDimsOfTensor:rhs axis:1 name:nil];
            }
            rhsExpanded = true;
            rhsRank = 2;
        }
    ...
    // Outer product case: no contracting dims, both inputs now rank-2 after expansion.
    // (M,1) @ (1,N) -> (M,N). No squeeze needed since result has the correct rank.
    if (!result && lhsBatchDims.empty() && rhsBatchDims.empty() && lhsRank == 2 && rhsRank == 2 &&
        lhsContractingDims.empty() && rhsContractingDims.empty() && lhsExpanded && rhsExpanded) {
        // After expansion: LHS is (M,1), RHS is (1,N). Standard matmul gives (M,N).
    ...
    if (!result && lhsBatchDims.empty() && rhsBatchDims.empty() && lhsRank == 2 && rhsRank == 2 &&
        lhsContractingDims.size() == 1 && rhsContractingDims.size() == 1) {
        // After expansion, rank-1 inputs always contract on the standard dims:
        // - Expanded LHS (1,K) contracts on dim 1 (the original dim 0)
        // - Expanded RHS (K,1) contracts on dim 0 (unchanged)
        // For non-expanded inputs, use the original contracting dims.
        int64_t lhsContractDim = lhsExpanded ? 1 : lhsContractingDims[0];
        int64_t rhsContractDim = rhsExpanded ? 0 : rhsContractingDims[0];
    ...
    // Squeeze dimensions we added for rank-1 contraction (not outer product).
    // Outer product expands (M,) to (M,1) and (N,) to (1,N) -> result (M,N) needs no squeeze.
    if (result && (lhsExpanded || rhsExpanded) && !lhsContractingDims.empty()) {
        if (lhsExpanded && rhsExpanded) {
            // Vector-vector dot: (1,K) @ (K,1) -> (1,1), squeeze to scalar ()
            result = [ctx.graph squeezeTensor:result axes:@[@0, @1] name:nil];
        } else if (lhsExpanded) {
            // Vector-matrix: (1,K) @ (K,N) -> (1,N), squeeze dim 0
            result = [ctx.graph squeezeTensor:result axis:0 name:nil];
        } else {
            // Matrix-vector: (M,K) @ (K,1) -> (M,1), squeeze dim 1
            result = [ctx.graph squeezeTensor:result axis:1 name:nil];
        }
    }
