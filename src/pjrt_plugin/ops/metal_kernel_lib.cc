...
class MetalKernelLibKernel : public Primitive {
 public:
  ...
  bool is_equivalent(const Primitive& other) const override {
    auto& o = static_cast<const MetalKernelLibKernel&>(other);
    return libpath_ == o.libpath_ && kname_ == o.kname_ &&
        hash_name_ == o.hash_name_ && grid_ == o.grid_ && tg_ == o.tg_ &&
        by_threadgroups_ == o.by_threadgroups_ && buffers_ == o.buffers_ &&
...
void MetalKernelLibKernel::eval_gpu(
  ...
  for (const auto& c : constants_) {
    fclist.emplace_back(
        c.value.data(), to_mtl_type(c.type),
  ...
  auto* kernel = d.get_kernel(
      kname_, lib, hash_name_.empty() ? kname_ : hash_name_, fclist);
