    ...
    if (batchGroupCount > 1) {
        ...
        if (N % batchGroupCount != 0 || O % batchGroupCount != 0) {
            MPS_LOG_ERROR(
                ...
                "input batch %d or kernel output features %d\n",
                batchGroupCount, N, O);
            return false;
        }
        ...
        //   step 3: reshape -> (Nper, *spatial, G*C_in)
        {
            ...
            for (int i = 2; i < nd - 1; ++i) perm.push_back(i);  // spatial
        ...
        for (int i = 0; i < numSpatialDims; ++i) {
            if (inputDilation[i] <= 1) continue;
            if (paddingLow[i] >= 0 && paddingHigh[i] >= 0) continue;
            int axis = 1 + i;
        ...
        auto result = IsIdentityPermutation(outPerm)
                          ? groupOut
                          : mlx::core::transpose(groupOut, outPerm);
        values.emplace(ToKey(op->getResult(0)), std::move(result));
        return true;
    }
