...
def test_narrowed_float_constant(dtype) -> None:
    """f64 constants must be narrowed to f32 by value, not reinterpreted byte-wise.

    Regression test for https://github.com/tillahoffmann/jax-mps/issues/231.
    MLX has no float64, so `MlirTypeToMlxDtype` maps f64 to float32; the raw
    8-byte payload used to be handed straight to a `const float*` read, so
    pi came back as 3.37e12. The f32 case is the control.
    """
    if TEST_MODE == "cpu":
        pytest.skip("MPS-specific test skipped in CPU-only mode")
...
module @test {{
  func.func @main(%arg0: tensor<4xf32>) -> tensor<4xf32> {{
    %c = stablehlo.constant dense<3.141592653589793> : tensor<{dtype}>
    %0 = stablehlo.convert %c : (tensor<{dtype}>) -> tensor<f32>
...
"""
    ...
    x = numpy.array([1.0, 2.0, -1.0, 0.5], dtype=numpy.float32)
    buf = jax.device_put(x, devices[0])
    result = numpy.asarray(exe.execute([buf])[0])
    numpy.testing.assert_allclose(result, x / numpy.float32(numpy.pi), rtol=1e-6)
